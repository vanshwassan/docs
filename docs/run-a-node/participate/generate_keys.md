...
!!! tip
	The Key Dilution parameter defaults to 10,000. It represents how many ephemeral keys your node will generate at a time, throughout the validity period. For example, by default, your node would auto-generate a batch of 10,000 ephemeral keys every 10,000 rounds, with the first batch created upfront. To reduce the size of the participation key, set the key dilution value to roughly the square root of the range that the partkey is valid for.
